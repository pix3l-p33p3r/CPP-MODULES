...
/*   Updated: 2023/10/29 22:15:25 by elel-yak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void PhoneBook::show_details()
{
	...
	{
		if (index_contact.length() > 1 || std::stoi(index_contact) > 8 || std::stoi(index_contact) < 0)
		{
			std::cout << "Unvalid index" << std::endl;
		}
		else
		{
			...
			else
			{
				...
				std::cout << std::setw(21) << "First name"
						  << ": " << con[i].get_f_name() << std::endl;
				std::cout << std::setw(21) << "Last name"
						  << ": " << con[i].get_l_name() << std::endl;
				std::cout << std::setw(21) << "Nick name"
						  << ": " << con[i].get_n_name() << std::endl;
				std::cout << std::setw(21) << "Darkest secret"
						  << ": " << con[i].get_darkest_secret() << std::endl;
				std::cout << std::setw(21) << "Phone number"
						  << ": " << con[i].get_phone() << std::endl;
				std::cout << "#-------------------------------------------#" << std::endl;
			}
		}
	}
